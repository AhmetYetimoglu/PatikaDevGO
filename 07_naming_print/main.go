package main

/* var x = 10
var customerName
var Y = 100 */

func main() {

	// (Print - Println) - Printf

	/* 	{
		var c
	} */

	/* 	fmt.Println("Merhaba")
	   	fmt.Print("Merhaba")
	   	fmt.Println("")
	   	fmt.Printf("Merhaba") */

	/* 	name := "Arin" */

	/* 	fmt.Print(name)
	   	fmt.Println(name)
	   	fmt.Printf(name) */

	/* 	fmt.Print("Benim Adım", name)
	   	fmt.Println("")
	   	fmt.Println("Benim Adım", name)
	   	fmt.Printf("Benim Adım %v %T", name, name) 
		%v değişkenin değerini yazdırır
		%T değişkenin tipini yazdırır
	*/
		

	/* 	x := 100
	   	y := 20
	   	z := 30
	   	fmt.Printf("%b %d %o", x, y, z) 
		%b ikili sayı sistemine göre değeri dönüştürür.
		%d 10 lu sayı sistemine göre değeri dönüştürür.
		%z 8 li sayı sistemine göre değeri dönüştürür.
	*/

	/* name, age := "Arin", 5 */

	// fmt.Print("Benim Adım ", name, ", ve ben ", age, " yaşındayım.")
	// fmt.Println("Benim Adım", name, "ve ben", age, "yaşındayım.")
	// fmt.Printf("Benim Adım %v, ve ben %v yaşındayım", name, age)

	// VISIBILITY

	/* 	fmt.Println(x)
	   	myFunc() */

	/* 	var coin string  // count - customer - coin
	   	
		Go camel case isimlendirme kullanılır
	   	var coinType string
	   	var custName string
	   	
	   kısaltmalar büyük harflerle yazılır
	   	var URL // Url değil
	   	var HTTP // http değil "xyzHTTP"
	   	i , j , k */

	// benForDongusuDegiskeninİsmiyim

}
